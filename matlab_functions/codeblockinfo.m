function [cblkdata, coeffmats, cbstep, disto] = codeblockinfo( cblkfile )
...
% 8 - cblk_w
% 9 - cblk_h
% 10 - encoded/decoded coding passes
% 11 - base stepsize
% 12 - codeblock unquantized variance
% 13 - codeblock quantized variance
%
%  Disto distortion for each pass goes from most to least sig bit
...
while n0 < length(F1)
    ...
    %Get distortion info if there
    if F1{n0}(1)=='D'
       tmp = strsplit(F1{n0}(2:end),' ');
       disto{n2} = cellfun(@(x) str2double(x),tmp(1:end-1));
       disto{n2}(end) = 0;
       n0 = n0 + 1;
    end
    ...
    if n1==7
        ...
        if ln(1)=='C'
            ...
            %bps = floor((str2double(temp(5))+2)/3);
            bps = str2double(temp(5));
            bss = str2double(temp(6));
            cbsteps = temp2(3:3:end);
            
            if ~isempty(strfind(cblkfile,'decoder'))
