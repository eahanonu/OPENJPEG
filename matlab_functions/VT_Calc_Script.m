...
for n0 = 1:size(encdata,1)
    resno = encdata(n0,2);
    comp = encdata(n0,1);
    sig2_uq = encdata(n0,12)/8192;
    sig2_q = encdata(n0,13)/8192;
    band = encdata(n0,3);
    cstep = encdata(n0,11);
   ...
   coefvt{n0} = zeros(size(decsteps{n0}));
    ...
    resno = decdata(n0,2);
    comp = decdata(n0,1);
    sig2_uq = decdata(n0,12)/8192;
    sig2_q = decdata(n0,13)/8192;
    band = decdata(n0,3);
    cstep = decdata(n0,11);
...
for comp = 0:2
    ...
    for n0 = 1:size(t1,1)
        ...
        %varinfo(t1(n0,5)+1:t1(n0,5)+t1(n0,9),t1(n0,4)+1:t1(n0,4)+t1(n0,8),comp+1) = t3(n0,11)*ones(size(t4{n0,1}));
